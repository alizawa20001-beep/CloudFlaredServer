...
const TOKEN = "cfut_f3iz4LcdokmVuyZMwru7tgzfuZwRJOWmQIloVCOg532d78cd"; // <-- PASTE YOUR TOKEN
const ACCOUNT_ID = "87a058e1b6de51a50099e1e4467ff611"; // <-- PASTE YOUR ACCOUNT ID

async function fetchPermissionGroups({ accountId, teken }) {
  ...
  const res = await fetch(
    ...
    {
      headers: { Authorization: `Bearer ${teken}` },
    },
  );
...
async function createWorkerTokenWithPermissions(permissionNames) {
  ...
  // 1. Fetch real permission groups
  const allPermissions = await fetchPermissionGroups();

  // 2. Find the permission IDs we need
...
async function listAvailablePermissions() {
  ...
  permissions.forEach((p) => {
    if (p.name.includes("Workers Scripts"))
      categories["Workers Scripts"].push(p.name);
    else if (p.name.includes("Workers Routes"))
      categories["Workers Routes"].push(p.name);
    ...
    else if (p.name.includes("Workers AI"))
      categories["Workers AI"].push(p.name);
    else if (p.name.includes("Workers") && !p.name.includes("Scripts"))
      categories["Other Workers"].push(p.name);
  });
...
async function CreateConnections() {
  ...
  if (command === "create") {
    // Use ONLY the permissions that exist in your account
    ...
    if (result.success) {
      ...
      console.log(`CLOUDFLARE_API_TOKEN=${result.tokenValue}`);
...
╠════════════════════════════════════════════════════════════╣
║  Commands:                                                 ║
║    node cloudtest.js list     - Show all permissions      ║
║    node cloudtest.js create   - Create full worker token  ║
╚════════════════════════════════════════════════════════════╝
    `);
}
